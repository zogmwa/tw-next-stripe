  ...
  return (
    <div className="mt-10 ml-3">
        ...
        <div className="md:flex md:justify-start">
          ...
          <Switch.Group className="mt-2 md:mt-0 md:ml-2">
            <Switch enabled={isCon} setEnabled={setIsCon} />
              <Switch.Label>
                {isCon ? 'Show cons as well' : 'Show cons too'}
              </Switch.Label>
          </Switch.Group>
        </div>
        <Button
      ...
      <div className="mt-6 md:mt-2">
              ...
              if (isCon)
                return (
                  ...
                  >
                    <Button
                      size="small"
                      className={attribute.is_con ? (isVoted ? "self-start text-background-light bg-text-error border-text-error" : "self-start text-text-error border-text-error") : (isVoted ? "self-start text-background-light bg-success border-success" : "self-start text-success border-success")}
                      icon={<HiChevronUp className={attribute.is_con ? (isVoted ? "text-background-light" : "text-text-error") : (isVoted ? "text-background-light" : "text-success")} />}
                    >
                ...
                if (!attribute.is_con)
                  return (
                    ...
                    >
                      <Button
                        size="small"
                        className={isVoted ? "self-start text-background-light bg-primary" : "self-start text-black border-text-tertiary"}
        ...
        {(attributes.length > 0 && (showedCount === 0 || showedCount >= defaultShowCount)) ? (viewMore ? (
            <div 
      ...
      <h1 className="text-base font-medium text-text-primary">Used by Compaines like</h1>
        <Carousel 
          buttonsShown={false}
          className="mt-2"
          itemsContainerClassName="border-none"
        >
